---
name: audit-recon-links
description: 使用高性能的命令工具对目标网站进行爬取以获取更多有用的网页链接.
---

# 背景

在确定目标网站存在http服务后, 需要主动爬取相关页面, 以获取更多有用的连接,这些链接将为后续的安全评估土工更多的线索.

## 例子

### Recon (Active Crawl Links)
- `katana -l artifacts/targets.txt -aff -jc -iqp -hl -nos -c 50 -p 50 -j -o artifacts/katana.jsonl`

### Recon (Active + Passive JS Links)
...
(注意这个例子使用了 docker run ,实际情况,应当避免在使用docker)

## [katana readme](https://github.com/projectdiscovery/katana/blob/dev/README.md)
<h1 align="center">
