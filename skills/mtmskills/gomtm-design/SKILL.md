...
6. datatable 采用 column-first + layout-contract 约定：桌面表格和移动摘要共用同一份 `ColumnDef<TData>[]`，桌面列尺寸必须进入 `ColumnDef` sizing 或 `columnDef.meta.table`，由表格壳层统一落到 `colgroup`/CSS width；不要靠肉眼猜多个固定 `w-*` 来修复列宽。
7. 表格 cell 先选公共 cell 原语，再写业务专用 cell；复制、长文本、指标、状态、动作都必须有明确列宽、`min-w-0`、截断/换行、稳定反馈和浅色/暗色主题状态。修复表格视觉问题必须用真实浏览器截图验收。
...
- 表格复制、长文本、指标、状态、动作：优先使用 `components/common/table/cells` 下的公共 cell；只服务单个业务表格的特殊 cell 留在当前子包，但必须继承公共 cell 的溢出和稳定尺寸约束。
