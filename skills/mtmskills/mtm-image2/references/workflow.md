...
4. 如果考虑使用内置 `imagegen`，先确认它会调用 GPT Image 或兼容图片模型；否则不要使用它做最终成图。
5. 不因本地渲染工具可用而绕过图片 API；HTML/SVG/Canvas/浏览器截图最多是用户明确要求的版式稿。
6. 命令返回后报告保存路径。
7. 修订时基于上一次 prompt 和图片路径继续，不从模糊需求重来。
...
- Codex app 转向本地绘图：停止该路径，改为 `scripts/mtm_image2.py`、可用宿主图片工具或 prompt-only。
